...
uses
  ...
  ToolWin, ComCtrls, AdvGroupBox, AdvToolBtn, ToolPanels, FolderDialog, AdvGrid,
  Grids, BaseGrid, WZArchive, WZDirectory, Generics.Collections, WZIMGFile,
  AdvObj, KeyHandler, WZReader, StrUtils, PngImage, Jpeg, {, Reactor,}
  Footholds, bass, BassHandler, MapPortal, AdvUtil, Mob2, Npc, {UI}
  MapleCharacter, {Boss,} Vectors2px, AbstractTextures, AbstractDevices,
  AbstractCanvas, AsphyreTimer, AsphyreSprite, AsphyreKeyboard, AsphyreFontsAlt,
  DirectInput, AsphyreFactory, DX9Providers, AsphyreTypes, Global, DX9Textures,
  AsphyreRenderTargets, LockRenderTarget, MapleMap, WzUtils, System.Types, Vcl.Buttons,
  scControls, scExtControls;
...
uses
  AddMobUnit, SaveMapUnit, ImageInfoUnit, RenderFormUnit, MapBack, MobInfo,
  AvatarUnit, ShowOptionUnit, Tools, AddNpcFormUnit, ChairformUnit,
  MorphFormUnit, MedalTagFormUnit,NickNameTagFormUnit, DamageSkinFormUnit, WorldMapFormUnit,
  CashFormUnit, TamingMobFormUnit, NameTag, MapleEffect, TamingMob,MapleChair,Pet,LabelRingFormUnit,PetFormUnit,FamiliarFormUnit,MonsterFamiliar;
{$R *.dfm}
...
begin
  ...
  begin
    ...
    begin
      ShowMessage('Wrong Folder, .wz file not found');
    end;
  end;
  ActiveControl := nil;
end;
...
begin
  ...
  begin
    MessageDlg('Old Maple not supported', mtInformation, [mbOK], 0);
    Exit;
  end
  else
    FamiliarForm.Show;

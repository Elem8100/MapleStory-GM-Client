...
type
  ...
  public
    ...
    property BlendMode:  TBlendingEffect read FBlendMode write FBlendMode;
...
begin
  ...
  FDoCollision := False;
  FMoved := True;
  FBlendMode := TBlendingEffect.Normal;
  FVisible := True;
  TruncMove := True;
  FTag := 0;
  FCollideMode := cmRect;
...
begin
  Result := FImageLib[FImageEntry].Parameters.Width;
end;

function TSprite.GetImageHeight: Integer;
begin
  Result := FImageLib[FImageEntry].Parameters.Height;
end;
...
begin
  ...
  case FTruncMove of
    True:
      ...
      FEngine.FCanvas.Draw(FImageLib[FImageEntry], Round(FX + FWorldX - FEngine.FWorldX), Round(FY + FWorldY - FEngine.FWorldY), False,  FBlendMode);

    False:
      FEngine.FCanvas.Draw(FImageLib[FImageEntry], FX + FWorldX - FEngine.FWorldX, FY + FWorldY - FEngine.FWorldY,  False,  FBlendMode);
...
begin
  ...
  case FDrawMode of
    ...
    0:
      case TruncMove of
        True:
          FEngine.FCanvas.DrawColor1(FImageLib[FImageEntry], Round(FX + FWorldX + Offset.X - FEngine.FWorldX), Round(FY + FWorldY + Offset.Y - FEngine.FWorldY),  FMirrorX,ARGB(FAlpha,FRed,FGreen,FBlue),  FBlendMode);

        False:
          FEngine.FCanvas.DrawColor1(FImageLib[FImageEntry], FX + FWorldX + Offset.X - FEngine.FWorldX, FY + FWorldY + Offset.Y - FEngine.FWorldY,  FMirrorX,ARGB(FAlpha,FRed,FGreen,FBlue), FBlendMode);

      end;

      // 1 color mode+Rotation
      1:
      FEngine.FCanvas.DrawRotateC(FImageLib[FImageEntry], FX + FWorldX + Offset.X - FEngine.FWorldX, FY + FWorldY + Offset.Y - FEngine.FWorldY, FAngle,1,  FBlendMode);
    {
      //4 color mode
      ...
      begin
        ...
        begin
          ...
          case FTileMode of
            tmHorizontal:
              begin
                FEngine.FCanvas.Draw(FImageLib[ImageEntry], cx * ChipWidth + OfsX - Offset.X, _y - Offset.Y,  FMirrorX,  FBlendMode);
              end;
            tmVertical:
              begin
                FEngine.FCanvas.Draw(FImageLib[ImageEntry], _x - Offset.X, cy * ChipHeight + OfsY - Offset.Y, FMirrorX, FBlendMode);
              end;
            tmFull:
              begin
                FEngine.FCanvas.Draw(FImageLib[ImageEntry], cx * ChipWidth + OfsX - Offset.X, cy * ChipHeight + OfsY - Offset.Y,  FMirrorX,  FBlendMode);
              end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        // if c >= 0 then
        if DrawMode = 0 then
          FEngine.FCanvas.Draw(FImageLib[FImageEntry], cx * ChipWidth + OfsX - Offset.X, cy * ChipHeight + OfsY - Offset.Y,  FMirrorX,  FBlendMode);

        if DrawMode = 1 then
          FEngine.FCanvas.DrawRotateC(FImageLib[FImageEntry], cx * ChipWidth + OfsX - Offset.X, cy * ChipHeight + OfsY - Offset.Y, FAngle,1, FBlendMode);
...
begin
  BlendMode := TBlendingEffect.Normal;
end;

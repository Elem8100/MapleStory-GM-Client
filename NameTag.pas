...
uses
  MapleCharacter, MapleMap, AsphyreTypes, DX9Textures;
...
begin
  ...
  begin
    WX := Round(Player.X) - Round(Engine.WorldX);
    WY := Round(Player.Y) - Round(Engine.WorldY);
    GameCanvas.Draw(AvatarTargets[TargetIndex], WX - 100, WY+5, 1, False, 255, 255, 255, 255);
  end;
  if IsReDraw then
    IsReDraw := False;
end;
...
begin
  ...
  begin
    ...
    var WestX := 100 - (CenterLength + EastWidth + WestWidth) div 2;
...
begin
  ...
  G := GetG(FontColor);
  B := GetB(FontColor);
  TargetIndex := AvatarTargets.Add(1, 200, 100, apf_A8R8G8B8, True, True);
  GameDevice.RenderTo(TargetEvent, 0, True, AvatarTargets[TargetIndex]);
...
begin
  ...
  begin
    WX := Round(Player.X) - Round(Engine.WorldX);
    WY := Round(Player.Y) - Round(Engine.WorldY);
    GameCanvas.Draw(AvatarTargets[TargetIndex], WX - 100, WY - 150, 1, False, 255, 255, 255, 255)
  end;
  if IsReDraw then
    IsReDraw := False;
end;
...
begin
  ...
  begin
    WX := Round(Player.X) - Round(Engine.WorldX);
    WY := Round(Player.Y) - Round(Engine.WorldY);
    GameCanvas.Draw(AvatarTargets[TargetIndex], WX - 100, WY - 28, 1, False, 255, 255, 255, 255)
  end;
  if IsReDraw then
    IsReDraw := False;
end;
